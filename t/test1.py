    ...
    def __create_graph(self):
        ...
        if Path("recode/recode_mat.{}.pickle".format(self._wm_c)).is_file():
            ...
            with open("recode/recode_mat.{}.pickle".format(self._wm_c), "r") as f:            
                recode_mat = tf.constant(pickle.load(f))
        else:
        ...
        sem_outputs = tf.reshape(                
                tf.matmul(tf.reshape(recode_mat, [self._n_states * self._wm_c,self._wm_c]), self.__sem_target),
                    ...
                    tf.concat(
                        [                            
                            tf.matmul(
                                tf.reshape(
                                    tf.matmul(
                                        tf.reshape(recode_mat, [self._n_states * self._wm_c, self._wm_c]), 
...
#        print(results[3])
#        print(results[4])    
#        print(results[3] * (1 - self._sigma) + results[4] * self._sigma)
#        print(self._sigma)
#        exit(0)
